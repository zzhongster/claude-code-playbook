...
2026-08-29 企业样本去重项目，算力机 192.168.1.24（72 核 / 251 GB，同时跑生产 trade-ai）。评审在试跑前一小时就指出三条：(a) DuckDB 从未设过 `memory_limit`，默认 ≈201 GiB > 100 GiB 的 `ulimit -v`，真到量是 malloc 失败而不是落盘；(b) `-v` 是每进程上限，40 个 worker 各 100 G 等于没限；(c) `taskset -c 0-39` 在 HT 机器上覆盖了全部物理核，生产只剩超线程兄弟。我把修复排在试跑之后，先放大样本跑了。

随后机器失联（端口能连、ssh 握手被关、生产 HTTP 无响应），我**误判**为批处理压死了机器——实际是机房停电。教训有两条：资源风险要先修再跑，这条不因停电而失效；**失联的归因要有证据**（`dmesg`、`free`、监控），"时间点吻合"不是证据，在拿到证据前对用户只能说"疑似"，不能说"是"。
