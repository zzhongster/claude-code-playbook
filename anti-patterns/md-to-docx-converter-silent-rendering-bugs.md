...
2026-09-13 已把四个修复同步回 skill 原始资产（文件头注明修复内容与验收建议），并用一份覆盖表头、两个独立有序列表、引用块加粗、中英文续行、嵌套子项、列表后普通段落的回归 markdown 转 PDF 验证通过。教训是：**修在工作副本上不算修完**——一次性副本随会话消失，下一份报告会从原件原样再踩一遍。
