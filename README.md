...
| [生成配置只过 check 不真跑](anti-patterns/generated-config-validated-by-check-not-run.md) | sing-box 1.14 删了旧 DNS 格式，员工手机拒载；修复版 `check` 全绿上线，`run` 才 FATAL（`detour to an empty direct outbound`）。给自动升级的客户端下发生成配置，验证=最新版+仍在野最低版两个真实二进制真跑+真请求（假 UUID 会把服务端掐连接伪装成 `EOF`）；WARN 出现即建到期项，迁移目标受最低版约束不受文档约束 |
