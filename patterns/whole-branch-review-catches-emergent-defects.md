...
| **slice-6a 输出黑盒化** | `enrich_company` 仍把 `bvdid`/`bvd_dict`（Bureau van Dijk=Orbis 东家）吐给 AI，泄露数据提供商身份。**连本薄片专门造的黑盒扫描器都没抓到**——它的禁词表恰好不含 `bvd`。 | 涌现 + **验证工具自身有盲区** | not_found note 和 dict_version 两处未被扫描器枚举；扫描器的禁词表不完整，形成"绿灯放行泄露响应"的假安全 |

三次，per-task 评审都判"通过"，都是终审在**整分支 diff + 真实数据抽样**里挖出来的。尤其 slice-6a：连本薄片**专门写来防泄露的扫描器自己**都有盲区，只有整分支终审这一层独立视角发现了它。一次是巧合，三次是规律——**任何单点验证（含你新造的那个测试）都可能和被测代码共享同一个盲区；终审是唯一不共享该盲区的独立层。**
